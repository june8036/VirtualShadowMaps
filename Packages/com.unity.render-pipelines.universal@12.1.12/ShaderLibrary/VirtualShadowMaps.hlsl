...
float3 GetVirtualShadowTexcoord(float3 worldPos, float3 normalWS)
{
	...
	float scale = (1.0f - clamp(dot(normalWS, light.direction.xyz), 0.0f, 0.9f)) * page.w;
...
real SampleVirtualShadowMap_PCF3x3(float4 coord, float2 receiverPlaneDepthBias)
{
    ...
    sum += uw[0] * vw[0] * real(SAMPLE_TEXTURE2D_SHADOW(_VirtualShadowTileTexture, sampler_VirtualShadowTileTexture, combineVirtualShadowcoordComponents(base_uv, float2(u[0], v[0]), coord.z, receiverPlaneDepthBias)));
    sum += uw[1] * vw[0] * real(SAMPLE_TEXTURE2D_SHADOW(_VirtualShadowTileTexture, sampler_VirtualShadowTileTexture, combineVirtualShadowcoordComponents(base_uv, float2(u[1], v[0]), coord.z, receiverPlaneDepthBias)));
    sum += uw[0] * vw[1] * real(SAMPLE_TEXTURE2D_SHADOW(_VirtualShadowTileTexture, sampler_VirtualShadowTileTexture, combineVirtualShadowcoordComponents(base_uv, float2(u[0], v[1]), coord.z, receiverPlaneDepthBias)));
    sum += uw[1] * vw[1] * real(SAMPLE_TEXTURE2D_SHADOW(_VirtualShadowTileTexture, sampler_VirtualShadowTileTexture, combineVirtualShadowcoordComponents(base_uv, float2(u[1], v[1]), coord.z, receiverPlaneDepthBias)));
