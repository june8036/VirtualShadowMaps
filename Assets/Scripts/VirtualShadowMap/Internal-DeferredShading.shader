...
#   ifdef _VIRTUAL_SHADOW_MAPS
    ...
    float virtualShadow = SampleVirtualShadowMap_PCF5x5(float4(shadowCoord, 0), 0);
    shadowMaskAttenuation = min(shadowMaskAttenuation, virtualShadow);
#if  defined(SHADOWS_SCREEN)
    realtimeShadowAttenuation = lerp(min(realtimeShadowAttenuation, virtualShadow), realtimeShadowAttenuation, _VirtualShadowBiasParams.z);
#endif
#endif
