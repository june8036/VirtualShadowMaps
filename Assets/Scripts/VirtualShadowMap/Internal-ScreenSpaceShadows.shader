...
fixed4 frag_pcf5x5(v2f i) : SV_Target
{
	...
	float3 shadowCoord = GetVirtualShadowTexcoord(wpos);

#if UNITY_USE_RECEIVER_PLANE_BIAS
	float fractionalSamplingError2 = 2 * dot(_VirtualShadowTileTexture_TexelSize.xy, abs(receiverPlaneDepthBias));
	shadowCoord.z -= min(fractionalSamplingError2, UNITY_RECEIVER_PLANE_MIN_FRACTIONAL_ERROR);
#endif

	shadow *= SampleVirtualShadowMap_PCF5x5(float4(shadowCoord, 0), receiverPlaneDepthBias);
#endif
...
SubShader {
	...
	Pass {
		...
		#pragma multi_compile __ _VIRTUAL_SHADOW_MAPS
